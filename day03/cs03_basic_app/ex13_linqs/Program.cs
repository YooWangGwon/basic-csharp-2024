...
namespace ex13_linqs
{
    ...
    {
        private int age;
        public string Name { get; set; }    // 자동 프로퍼티 -> 값에 아무 처리를 안할거기 때문
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Product[] arrProduct =
            {
                ...
                new Product { Title = "오징어게임", Star = "전지현"},
            ...
            {   // GroupKey가 True인가 False인가
                if (group.GroupKey == true) 
                { 
                    Console.WriteLine($"- 175cm 미만? : {group.GroupKey}");
                    foreach (var profile in group.Profiles) 
                    {
                        Console.WriteLine($">>>{profile.Name}({profile.Age}세) {profile.Height}");
                    }
                }
            }
            ...
            {
                Console.WriteLine($"작품 : {item.Work}\n이름 : {item.Name} / 나이 : {item.Age}\n");
            }
